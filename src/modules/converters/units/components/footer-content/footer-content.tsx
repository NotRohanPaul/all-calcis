...
const FooterContent = () => {
    ...
    const [currentCategory, setCurrentCategory] = useState<string>(() => (getCurrentCategory(converterState) ?? unitsDetailsList[0].category));

    useEffect(() => {
        setCurrentCategory(getCurrentCategory(converterState) ?? unitsDetailsList[0].category)
    }, [converterState])
